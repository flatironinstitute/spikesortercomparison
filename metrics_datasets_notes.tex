...
\item {\bf Accuracy vs ground-truth} (Class I).
  ...
  We now consider a single GT unit.
  Let $\{t_j^\tbox{GT}\}_{j=1}^{n_\tbox{GT}}$ be the set of its true firings.
  Usually
  ...
  Note: we decided to avoid optimizing for a bijective map from
  the set of all GT units to sorted units, which requires
  computation of a confusion matrix and the Hungarian algorithm.
  With firing time
  information this becomes algorithmically complex \cite{validspike},
  and is not relevant unless there are a large number of poorly-sorted
  GT units to match.
