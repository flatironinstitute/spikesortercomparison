...
\begin{abstract}
  ...
  It also includes discussion and some subjective opinions. \com{(jfm) suggest removing this sentence about subjective opinions}
...
\com{(jfm) Note that the last item in that list runs into the margin in the rendered pdf}
...
\begin{minipage}[t]{1in}Inputs:\end{minipage} \begin{minipage}[t]{5in}
  ...
  $T$ (number of time-points), together with information on the sample rate (samples per second) and the 2D electrode coordinate locations in $\mu$m. Other dataset-specific attributes such as the expected polarity of spikes (positive, negative, or both) may also be provided, which may or may not be used by the spike sorter.
  
  \end{minipage}
  
  \com{(jfm) I removed discussion of file formats here because this is the math description}
...
can be derived from the union of this input and output data. [[Spike sorters are thus not
...
\com{(jfm) We need to be careful here... are we requiring that this channel information
provided by the sorter be consistent with the method of returning the peak channel?
There is still more to think about regarding this particular output.
}
\item
...
\com{(jfm) I suggest removing the last sentence about drawing independent samples}
\een
...
the meta data. Thus we have the advantage of not requiring a single format at the
...
[[jfm says: This does not really specify the file format. I suggest
we provide various options including .mda and .hdf5.
]]
